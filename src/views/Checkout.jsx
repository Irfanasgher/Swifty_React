...
class Checkout extends Component {
  ...
  confirmOrderHandler = (event,shoppingTotal) => {
    ...
    let order = {};
    order["totalCost"] = shoppingTotal;
    order["itemName"] = this.props.productsProps;
    order["cart"] = this.props.cartProductsProps;
    order["user"] = {
      name: this.state.customerInfo.name.value,
      phone: this.state.customerInfo.phone.value,
      unit: this.state.customerInfo.unit.value,
      postal: this.state.customerInfo.postal.value,
    };
    order["usedPromoCode"] = this.state.promoCode;
    order["currency"] = this.props.usedCurrencyProp;
    order["shopName"] = this.props.shopNameProp;
    order["paymentMethod"] = this.state.paymentMethod;
    order["deliveryOption"] = this.state.usedDeliveryOption;

    // if wanna use credit card need use stripe token for payments
  ...
  render() {
    ...
    let shoppingTotal =
      productTotals > 0
        ? productTotals + shippingPrice - discountAmount
        : 0;
    ...
    if (this.state.paymentMethod === "creditCard") {
      ...
      chosenPaymentMethod = (
        <div className={"ml-4 p-3"}>
          You will pay when the product is delivered to you. Either Cash/PayNow/PayLah
        </div>
      );
    }
...
Checkout.propTypes = {
  ...
  shopNameProp: PropTypes.string.isRequired
};
